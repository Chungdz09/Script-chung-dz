...
getgenv().configs.PlayerEsp = not getgenv().configs.PlayerEsp -- Toggle mỗi lần chạy

-- Xoá ESP cũ nếu có
if getgenv()._espObjects then
    for _, esp in pairs(getgenv()._espObjects) do
        if esp.connection then esp.connection:Disconnect() end
        for _, obj in pairs(esp.drawings) do
            if obj then obj:Remove() end
        end
    end
    getgenv()._espObjects = nil
end
...
function EspOnPlayer(target)
	...
	conn = RunService.RenderStepped:Connect(function()
		...
		if IsPlayerAlive(target) and target ~= LocalPlayer then
			...
			NameText.Text = target.DisplayName
			NameText.Position = Vector2.new(namePos.X, namePos.Y)
	...
	table.insert(getgenv()._espObjects, {
		connection = conn,
		drawings = { Box, BoxOutline, HPBar, HPBarOutline, NameText }
	})
end
